...
export function plantBomb(player, playerId) {
    // Läheb perse kui mängulauda nihutada.
    const playerRect = player.getBoundingClientRect()

    canMoveThroughBomb = true;

    let explosionTileY = Math.round((playerRect.top - 24) / tileSize) - 1;
    let explosionTileX = Math.round(playerRect.left / tileSize);
    console.log(explosionTileX, explosionTileY)
